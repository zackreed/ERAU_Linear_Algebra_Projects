function interactive_plane_plot_3d_draggable(varargin)
    ...
    %point_text = uicontrol('Style', 'text', 'Units', 'normalized', 'Position', [0.02 0.83 0.3 0.03], ...
    ...
    function dragPoint(~, ~)
        ...
        new_z = current_point(1, 3);
    
        % Update the center point position
        initial_point = [new_x, new_y, new_z];
        set(point_handle, 'XData', new_x, 'YData', new_y, 'ZData', new_z);
        set(point_text, 'String', sprintf('Point: (%.2f, %.2f, %.2f)', new_x, new_y, new_z));
    
        % Update the plot with the new point position
        update_plot();
    end
    ...
    % Add the ButtonDownFcn to the point handle
    set(point_handle, 'ButtonDownFcn', @(src, event) startDragPoint(src));
    ...
    function update_plot()
        ...
        current_point = get(gca, 'CurrentPoint');
        point_x = get(current_point(1, 1), 'Value');
        point_y = get(current_point(1, 2), 'Value');
        point_z = get(current_point(1, 3), 'Value');
